...
describe('dateUtils', () => {
  ...
  describe('date formatting functions', () => {
    ...
    describe('toDisplayDate', () => {
      ...
      it('should throw error for invalid date string', () => {
        expect(() => toDisplayDate('invalid-date')).toThrow('Invalid date format');
      });
    });
  ...
  describe('date calculation functions', () => {
    ...
    describe('getDateMonthsFromNow', () => {
      it.each([
        ...
        [-1, '2024-12-24'],
        [0, '2025-01-24']
      ])('should calculate %i months from now correctly', (months, expected) => {
    ...
    describe('getDateYearsAgo', () => {
      ...
      it('should throw error for negative years', () => {
        expect(() => getDateYearsAgo(-1)).toThrow('Years must be a positive number');
      });
    });
    ...
    describe('getDaysDifference', () => {
      it.each([
        ...
        ['2025-01-24', null, 0],
        [null, '2025-01-24', 0]
      ])('should calculate difference between %s and %s as %i days', (date1, date2, expected) => {
  ...
  describe('date comparison functions', () => {
    ...
    describe('isWithinDays', () => {
      it.each([
        ...
        ['2025-01-20', 2, false],
        [null, 2, false]
      ])('should check if %s is within %i days correctly', (date, days, expected) => {
        // Ensure we're using the same time of day for comparison
        if (date) {
          const testDate = new Date(date);
          testDate.setUTCHours(12, 0, 0, 0);
          expect(isWithinDays(testDate.toISOString(), days)).toBe(expected);
        } else {
          expect(isWithinDays(date, days)).toBe(expected);
        }
      });
    });
  });
  ...
  describe('convertDateToUTC', () => {
    ...
    it('should throw error for invalid date', () => {
      expect(() => convertDateToUTC(new Date('invalid'))).toThrow('Invalid Date object');
    });
  });
});
